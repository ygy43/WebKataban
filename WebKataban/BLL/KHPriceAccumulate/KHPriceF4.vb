...
Module KHPriceF4
    ...
    Public Sub subPriceCalculation(ByVal objKtbnStrc As KHKtbnStrc, _
        ...
        Try
            ...
            'Z軸ストロークの値を求める(中間STまるめ処理)
            Select Case True
                Case CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim) <= 30
                    intZStroke = 30
                Case 31 <= CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim) And CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim) <= 50
                    intZStroke = 50
                Case 51 <= CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim) And CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim) <= 70  '51～70mm
                    intZStroke = 70
                Case 71 <= CInt(objKtbnStrc.strcSelection.strOpSymbol(5).Trim)
                    intZStroke = 100
            End Select
            ...
            Select Case objKtbnStrc.strcSelection.strOpSymbol(1).Trim
                Case "1"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                    strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & strKSymbol & CdCst.Sign.Hypen & intZStroke.ToString
                    decOpAmount(UBound(decOpAmount)) = 1
                Case "2"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                ...
                Case "3", "4"
                    Select Case objKtbnStrc.strcSelection.strOpSymbol(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-SC-" & strKSymbol
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                ...
                Case "A"
                    Select Case objKtbnStrc.strcSelection.strOpSymbol(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & objKtbnStrc.strcSelection.strOpSymbol(10).Trim
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                ...
                Case "Q"
                    Select Case objKtbnStrc.strcSelection.strOpSymbol(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & objKtbnStrc.strcSelection.strOpSymbol(11).Trim & CdCst.Sign.Hypen & strKSymbol
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
            ...
            Select Case objKtbnStrc.strcSelection.strOpSymbol(1).Trim
                Case "1"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                    strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-ATATCHMENT-" & strXSymbol
                    decOpAmount(UBound(decOpAmount)) = 1
                Case "2"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
