                ...
                If objKtbnStrc.strcSelection.strOpSymbol(4).Trim <> "" Then
                    Select Case ""
                        Case "T2YDT"
                            ...
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = Left(objKtbnStrc.strcSelection.strSeriesKataban.Trim, 3) & _
                                                                       objKtbnStrc.strcSelection.strOpSymbol(4).Trim & CdCst.Sign.Hypen & "DT"
                            decOpAmount(UBound(decOpAmount)) = KHKataban.fncSwitchQtyGet(objKtbnStrc.strcSelection.strOpSymbol(5).Trim)
                        Case Else
                            If objKtbnStrc.strcSelection.strOpSymbol(3).Trim.Length > 3 And _
                               Mid(objKtbnStrc.strcSelection.strOpSymbol(3).Trim, 4, 1) <> "H" And _
                               Mid(objKtbnStrc.strcSelection.strOpSymbol(3).Trim, 4, 1) <> "V" Then
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                strOpRefKataban(UBound(strOpRefKataban)) = Left(objKtbnStrc.strcSelection.strSeriesKataban.Trim, 3) & _
                                                                           objKtbnStrc.strcSelection.strOpSymbol(4).Trim & CdCst.Sign.Hypen & _
                                                                           Mid(objKtbnStrc.strcSelection.strOpSymbol(3).Trim, 4, 1)
                                decOpAmount(UBound(decOpAmount)) = KHKataban.fncSwitchQtyGet(objKtbnStrc.strcSelection.strOpSymbol(5).Trim)
                            Else
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
